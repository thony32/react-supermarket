...
export default function HistoriqueVente() {
  ...
  const recDate = () => {
    ...
    if (date1 !== "" && date2 !== "") {
      ...
      axios
        .get(`http://localhost:2000/recVenteDate/${date1}/${date2}`)
        .then((res) => {
  ...
  const handleSearch = (search: string | null) => {
    if (search) {
      axios
        .get(`http://localhost:2000/recRehetraDetVente/${search}`)
        .then((res) => {
          setHistory(res.data);
        });
    } else {
  ...
  const fetchData = () => {
    axios.get("http://localhost:2000/AfficherTousVente").then((res) => {
      setHistory(res.data);
    });
  };
  useEffect(() => {
    ...
    <div className="apple space-y-4">
      ...
      <div className="flex gap-4 px-4 lg:px-16">
        ...
        <input
          ...
          className="bg-gray-500 rounded-lg w-full"
        ></input>
        <button onClick={recDate} className="btn btn-info">Recherche</button>
      </div>
            ...
            <tbody>
              {history.map((d: any, i: any) => (
                <tr className="bg-gray-700" key={i}>
                  ...
                  <td className="px-6 py-4">{d.dateVente}</td>
                </tr>
              ))}
            </tbody>
          </table>
